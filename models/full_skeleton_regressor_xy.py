#!/usr/bin/env python3
"""
xyヒートマップから全骨格（22関節）座標を回帰するCNNモデル
"""
...
class FullSkeletonRegressorXY(nn.Module):
    """
    xyヒートマップから全骨格（22関節）座標を回帰するCNNモデル
    
    入力: xyヒートマップ (1, H, W) - 固定範囲、固定解像度
    出力: 全22関節の座標 (44次元: 22関節 × 2座標)
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            joint_coords: (B, num_joints * 2) - 全関節座標（正規化済み、0-1範囲）
                [Pelvis_x, Pelvis_y, L_Hip_x, L_Hip_y, ..., R_Wrist_x, R_Wrist_y]
        """
        # Encoder
...
def create_full_skeleton_regressor_xy(
    heatmap_size: int = 50,
    num_joints: int = 22,
    base_channels: int = 32,
    dropout: float = 0.5
) -> FullSkeletonRegressorXY:
    """
    xyヒートマップから全骨格（22関節）座標を回帰するモデルを作成
    
    Args:
        heatmap_size: ヒートマップの解像度（デフォルト: 50×50）
        num_joints: 関節数（デフォルト: 22）
        base_channels: ベースチャネル数
        dropout: Dropout率
    
    Returns:
        model: FullSkeletonRegressorXY
    """
    model = FullSkeletonRegressorXY(
        in_channels=1,
