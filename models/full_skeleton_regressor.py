#!/usr/bin/env python3
"""
xzヒートマップから全骨格（22関節）座標を回帰するCNNモデル
"""
...
class FullSkeletonRegressor(nn.Module):
    """
    xzヒートマップから全骨格（22関節）座標を回帰するCNNモデル
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            joint_coords: (B, num_joints * 2) - 全関節座標（正規化済み、0-1範囲）
                [Pelvis_x, Pelvis_z, L_Hip_x, L_Hip_z, ..., R_Wrist_x, R_Wrist_z]
        """
        # Encoder
...
def create_full_skeleton_regressor(
    heatmap_size: int = 50,
    num_joints: int = 22,
    base_channels: int = 32,
    dropout: float = 0.5
) -> FullSkeletonRegressor:
    """
    xzヒートマップから全骨格（22関節）座標を回帰するモデルを作成
