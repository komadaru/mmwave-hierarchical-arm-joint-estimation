#!/usr/bin/env python3
"""
xyヒートマップから腕の関節座標を回帰するモデルの学習スクリプト
"""
...
from heatmap_distal_detection.dataset_arm_joints_xy import ArmJointDatasetXY, collate_fn
...
class WeightedMSELoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.reduction == 'mean':
            loss = weighted_mse.sum() / (mask.float() * weights).sum()
...
class ArmJointTrainer:
    ...
    def __init__(
        ...
        # 出力ディレクトリ
        self.output_dir = Path(config.get('output_dir', 'outputs/arm_joint_models_xy'))
...
def main():
    parser = argparse.ArgumentParser(description='Train arm joint regressor from xy heatmap')
    parser.add_argument('--train_data', type=str, required=True,
                       help='Training data path (JSONL)')
    parser.add_argument('--val_data', type=str, required=True,
                       help='Validation data path (JSONL)')
    parser.add_argument('--output_dir', type=str, default='outputs/arm_joint_models_xy',
    ...
    parser.add_argument('--y_range', type=float, nargs=2, default=[3.5, 5.0],
                       metavar=('Y_MIN', 'Y_MAX'),
                       help='Fixed y-axis range (default: 3.5 5.0)')
    ...
    # モデル
    model = create_arm_joint_regressor_xy(
