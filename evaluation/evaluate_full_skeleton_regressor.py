#!/usr/bin/env python3
"""
xzヒートマップから全骨格（22関節）座標を回帰するモデルの評価スクリプト
"""
...
def compute_mpjpe(
    ...
    if valid_mask.sum() == 0:
        return {
            'mpjpe': np.nan,
            'mpjpe_per_joint': [np.nan] * 22,
            'valid_joints': 0
        }
    
    # 各関節の誤差を計算（正規化座標での誤差）
    num_joints = pred_coords.shape[1] // 2
    joint_errors = []
...
def compute_spatial_mpjpe(
    ...
    if valid_mask.sum() == 0:
        return {
            ...
            'spatial_mpjpe_per_joint_cm': [np.nan] * 22,
            'valid_joints': 0
        }
...
def evaluate(
    ...
    for i, joint_name in enumerate(joint_names):
        if i < len(norm_metrics['mpjpe_per_joint']):
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate full skeleton (22 joints) regressor from heatmap')
    ...
    # データセット
    if plane == 'xz':
        test_dataset = FullSkeletonDataset(
        ...
        test_dataset = FullSkeletonDatasetXY(
            data_path=args.test_data,
            x_range=x_range,
