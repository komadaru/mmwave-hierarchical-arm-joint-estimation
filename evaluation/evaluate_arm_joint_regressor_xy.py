#!/usr/bin/env python3
"""
xyヒートマップから腕の関節座標を回帰するモデルの評価スクリプト
"""
...
def denormalize_coords(
    ...
    for i in range(num_joints):
        ...
        # y座標
        coords[:, i*2+1] = coords[:, i*2+1] * (y_max - y_min) + y_min
...
def compute_spatial_mpjpe(
    ...
    """
    # 正規化座標を元の範囲に戻す
    pred_coords_m = denormalize_coords(pred_coords, x_range, y_range)
    gt_coords_m = denormalize_coords(gt_coords, x_range, y_range)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate arm joint regressor from xy heatmap')
    ...
    y_range = tuple(config.get('y_range', args.y_range))
