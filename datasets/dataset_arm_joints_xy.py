...
from heatmap_distal_detection.visualize_xz_heatmap import create_2d_heatmap


class ArmJointDatasetXY(Dataset):
    """
    xyヒートマップと腕の関節座標のデータセット
    
    入力: xyヒートマップ（固定範囲、固定解像度）
    ラベル: 腕の関節座標（xy平面に投影、固定範囲内で正規化）
    """
    ...
    def __init__(
        ...
        y_range: Tuple[float, float] = (3.5, 5.0),
        bins: int = 50,
        ...
        print(f"Fixed ranges: x={x_range}, y={y_range}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if not file_path or not os.path.exists(file_path):
            ...
            try:
                ...
                # xyヒートマップを作成（固定範囲）
                heatmap, _, _ = create_2d_heatmap(
                    points,
                    axis1='x',
                    axis2='y',
                    feature=self.feature,
                    bins=self.bins,
                    axis1_range=self.x_range,
                    axis2_range=self.y_range
                )
                ...
                if self.normalize_heatmap:
                    ...
                    # デバッグ: 正規化後の値を確認（最初の数回のみ）
                    if idx < 3:
                        valid_after = heatmap[valid_mask] if valid_mask.any() else np.array([])
                        if len(valid_after) > 0:
                            print(f"[Dataset] Sample {idx}: normalize_heatmap={self.normalize_heatmap}, after normalization: min={valid_after.min():.6f}, max={valid_after.max():.6f}, mean={valid_after.mean():.6f}")
                
                # float32に変換（モデルの重みと型を一致させるため）
                heatmap = heatmap.astype(np.float32)
                
                # 腕の関節座標を取得（JSONLファイルから直接読み込む）
                joint_coords = None
                if 'arm_joint_coords' in sample and sample['arm_joint_coords'] is not None:
                    # arm_joint_coordsは既にxy平面に投影された座標（正規化前）
                    arm_coords_raw = np.array(sample['arm_joint_coords'])  # (num_arm_joints * 2,)
                    # デバッグ: 最初の数サンプルで確認
                    if idx < 3:
                        print(f"[Dataset] Sample {idx}: arm_joint_coords found, length={len(arm_coords_raw)}, has_nan={np.isnan(arm_coords_raw).any()}")
                        print(f"  First few values: {arm_coords_raw[:6] if len(arm_coords_raw) >= 6 else arm_coords_raw}")
                    # 正規化を適用
                    joint_coords = []
                    for i in range(0, len(arm_coords_raw), 2):
                        if i + 1 < len(arm_coords_raw):
                            x, y = arm_coords_raw[i], arm_coords_raw[i + 1]
                            if not (np.isnan(x) or np.isnan(y)):
                                x_norm = self.normalize_coord(x, self.x_range)
                                y_norm = self.normalize_coord(y, self.y_range)
                                joint_coords.extend([x_norm, y_norm])
                            else:
                                joint_coords.extend([np.nan, np.nan])
                    joint_coords = np.array(joint_coords, dtype=np.float32)
                    if idx < 3:
                        print(f"  After normalization: has_nan={np.isnan(joint_coords).any()}, valid_count={np.sum(~np.isnan(joint_coords))}/{len(joint_coords)}")
                elif 'gt_joints' in sample and sample['gt_joints'] is not None:
                    # フォールバック: gt_jointsから抽出（後方互換性のため）
                    if idx < 3:
                        print(f"[Dataset] Sample {idx}: Using gt_joints (fallback)")
                    ...
                    for joint_idx in self.arm_joint_indices:
                        if joint_idx < len(gt_joints):
                            ...
                            x, y = joint_3d[0], joint_3d[1]  # xy平面に投影
                            # 正規化
                            x_norm = self.normalize_coord(x, self.x_range)
                            y_norm = self.normalize_coord(y, self.y_range)
                            joint_coords.extend([x_norm, y_norm])
                    ...
                    # どちらもない場合はNaN
                    if idx < 3:
                        print(f"[Dataset] Sample {idx}: WARNING - No arm_joint_coords or gt_joints found! Keys in sample: {list(sample.keys())}")
                    joint_coords = np.full((len(self.arm_joint_indices) * 2,), np.nan, dtype=np.float32)
